...
// HandlerCheckPopStatus verifica o status dos POPs de uma instancia,
// consultando a saude via ws2 e atualizando o status_timeout.
func HandlerCheckPopStatus(instancia entity.Instancia) error {
...
func processarPop(tag string, db *sql.DB, pop entity.Pop, chaveMestra []byte) error {
...
func atualizarOnline(db *sql.DB, pop entity.Pop) error {
	if pop.StatusTimeout > 0 {
		if err := repositorio.AtualizarStatusTimeout(db, pop.ID, 0, nil); err != nil {
			return err
		}
	}
	return nil
...
func atualizarOffline(db *sql.DB, pop entity.Pop) error {
	switch pop.StatusTimeout {
	case 3:
		dados := 4
		return repositorio.AtualizarStatusTimeout(db, pop.ID, dados, nil)
	case 2:
		dados := 3
		agora := fuso.Agora().Format("2006-01-02 15:04:05")
		return repositorio.AtualizarStatusTimeout(db, pop.ID, dados, &agora)
	default:
